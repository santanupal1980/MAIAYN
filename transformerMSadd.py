...
class MultiHeadAttention():
    ...
    def __call__(self, q, k, v, mask=None):
        ...
        if self.mode == 0:
            ...
            if mask!=None: # New changed masked conditionally
...
class PositionwiseFeedForward():
    def __init__(self, d_hid, d_inner_hid, dropout=0.1):
        self.w_1 = Conv1D(4*d_inner_hid, 1, activation='relu')
...
class JointEncoder():
    ...
    def __call__(self, src1_pos, src2_pos, enc1, enc2, return_att=False, active_layers=999):
        #x = concatenate([enc1, enc2], axis=1)
        x = Add()([enc1, enc2]) # ADD model
        #src_pos = concatenate([src1_pos, src2_pos], axis=1) # position encoding for joint encoder start
        src_pos = Add()([src1_pos, src2_pos]) # ADD model  for position encoding for joint encoder start
        if src_pos is not None:
            pos = self.pos_layer(src_pos)
            x = Add()([x, pos]) # position encoding for joint encoder end'''
        if return_att: atts = []
        mask = None # Lambda(lambda x: GetPadMask(x, x))(src_seq)
        for enc_layer in self.layers[:active_layers]:
            x, att = enc_layer(x, mask)
            if return_att: atts.append(att)
        return (x, atts) if return_att else x
...
class Decoder():
    ...
    def __call__(self, tgt_seq, tgt_pos, src1_seq, src2_seq, enc_output, return_att=False,
                 active_layers=999):
        ...
        enc1_mask = Lambda(lambda x: GetPadMask(x[0], x[1]))([tgt_seq, src1_seq])
        enc2_mask = Lambda(lambda x: GetPadMask(x[0], x[1]))([tgt_seq, src2_seq])

        ######################################## Joint ################################
        #enc_mask = concatenate([enc1_mask, enc2_mask], axis=-1)
        enc_mask = Add()([enc1_mask, enc2_mask])
...
class Transformer:
    def __init__(self, i_tokens1, i_tokens2, o_tokens, len_limit, d_model=256, \
                 d_inner_hid=512, n_head=4, d_k=64, d_v=64, layers=2, dropout=0.1, \
                 share_word_emb=False):
        self.i_tokens1 = i_tokens1
        self.i_tokens2 = i_tokens2
        self.o_tokens = o_tokens
        self.len_limit = len_limit
        self.src1_loc_info = True
        self.src2_loc_info = True
        self.d_model = d_model
        self.decode_model = None
        d_emb = d_model

        pos_emb = Embedding(len_limit, d_emb, trainable=True, \
                            weights=[GetPosEncodingMatrix(len_limit, d_emb)])
        #============== Input output embedding =============
        i_word_emb1 = Embedding(i_tokens1.num(), d_emb)
        i_word_emb2 = Embedding(i_tokens2.num(), d_emb)

        if share_word_emb:
            assert i_tokens2.num() == o_tokens.num()
            o_word_emb = i_word_emb2
        else:
            o_word_emb = Embedding(o_tokens.num(), d_emb)

        #======================== Encoding ===================================

        self.encoder1 = Encoder(d_model, d_inner_hid, n_head, d_k, d_v, layers, dropout, \
                                word_emb=i_word_emb1, pos_emb=pos_emb)

        self.encoder2 = Encoder(d_model, d_inner_hid, n_head, d_k, d_v, layers, dropout, \
                                word_emb=i_word_emb2, pos_emb=pos_emb)
        #=====================================================================

        # Now two encoders are act as inputs of the Joint encoder
        #========================================================
    ...
    def compile(self, optimizer='adam', active_layers=999):
        src1_seq_input = Input(shape=(None,), dtype='int32')  # for source
        src2_seq_input = Input(shape=(None,), dtype='int32')  # for MT
        tgt_seq_input = Input(shape=(None,), dtype='int32')  # for PE

        src1_seq = src1_seq_input  # A copy for SRC
        src2_seq = src2_seq_input  # A copy for MT
        tgt_seq = Lambda(lambda x: x[:, :-1])(tgt_seq_input)
        tgt_true = Lambda(lambda x: x[:, 1:])(tgt_seq_input)

        src1_pos = Lambda(self.get_pos_seq)(src1_seq)
        src2_pos = Lambda(self.get_pos_seq)(src2_seq)
        tgt_pos = Lambda(self.get_pos_seq)(tgt_seq)

        if not self.src1_loc_info: src1_pos = None
        if not self.src2_loc_info: src2_pos = None
        ...
        enc_output = self.jointencoder(src1_pos, src2_pos, enc1_output, enc2_output, active_layers=active_layers)  # Configuring jointencoder for MT
        ...
        self.model = Model([src1_seq_input, src2_seq_input, tgt_seq_input], loss)
        self.model.add_loss([loss])
        self.output_model = Model([src1_seq_input, src2_seq_input, tgt_seq_input], final_output)
    ...
    def make_src1_seq_matrix(self, input_seq):
        src_seq = np.zeros((1, len(input_seq) + 3), dtype='int32')
        src_seq[0, 0] = self.i_tokens1.startid()
        for i, z in enumerate(input_seq): src_seq[0, 1 + i] = self.i_tokens1.id(z)
        src_seq[0, len(input_seq) + 1] = self.i_tokens1.endid()
        return src_seq

    def make_src2_seq_matrix(self, input_seq):
        src_seq = np.zeros((1, len(input_seq) + 3), dtype='int32')
        src_seq[0, 0] = self.i_tokens2.startid()
        for i, z in enumerate(input_seq): src_seq[0, 1 + i] = self.i_tokens2.id(z)
        src_seq[0, len(input_seq) + 1] = self.i_tokens2.endid()
        return src_seq

    def decode_sequence(self, input1_seq, input2_seq, delimiter=' '):
        src1_seq = self.make_src1_seq_matrix(input1_seq)
        src2_seq = self.make_src2_seq_matrix(input2_seq)
        decoded_tokens = []
        target_seq = np.zeros((1, self.len_limit), dtype='int32')
        target_seq[0, 0] = self.o_tokens.startid()
        for i in range(self.len_limit - 1):
            output = self.output_model.predict_on_batch([src1_seq, src2_seq, target_seq])
    ...
    def make_fast_decode_model(self):
        src1_seq_input = Input(shape=(None,), dtype='int32')
        src2_seq_input = Input(shape=(None,), dtype='int32')

        tgt_seq_input = Input(shape=(None,), dtype='int32')
        src1_seq = src1_seq_input
        src2_seq = src2_seq_input
        ...
        enc_output = self.jointencoder(enc1_output, enc2_output)

        self.encode_model1 = Model(src1_seq_input, enc1_output)
        self.encode_model2 = Model(src2_seq_input, enc2_output)
        self.jointencoder_model = Model([src1_seq_input, src2_seq_input], enc_output)
    ...
    def beam_search(self, input1_seq, input2_seq, topk=5, delimiter=' '):
        if self.decode_model is None: self.make_fast_decode_model()
        src1_seq = self.make_src1_seq_matrix(input1_seq)
        src2_seq = self.make_src2_seq_matrix(input2_seq)
        src1_seq = src1_seq.repeat(topk, 0)
        src2_seq = src2_seq.repeat(topk, 0)
...
if __name__ == '__main__':
    ...
    X1, X2, Y = pad_to_longest(X1, itokens1), pad_to_longest(X2, itokens2), pad_to_longest(Y, otokens)
    print(X1.shape, X2.shape, Y.shape)

    s2s = Transformer(itokens1, itokens2, otokens, 10, 15)
